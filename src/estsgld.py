...
with open('/output/means.txt',"w") as f:
    f.write(str(means))
with open('/output/aleatoric.txt',"w") as f:
    f.write(str(aleatoric))
with open('/output/total_unc.txt',"w") as f:
    f.write(str(total_unc))
...
plt.title('Estimate Uncertainty', fontsize=40)
plt.tick_params(labelsize=30)
plt.xticks(np.arange(-4, 5, 2))
plt.yticks(np.arange(-4, 5, 2))
plt.gca().yaxis.grid(alpha=0.3)
plt.gca().xaxis.grid(alpha=0.3)
plt.savefig('/output/sgld.jpg', bbox_inches = 'tight')
plt.show()
